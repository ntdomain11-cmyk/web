...
async function ensureGoogleReviewsColumns() {
    const alters = [
        ...
        "ALTER TABLE siteconfig ADD COLUMN mapEmbedUrl TEXT NULL",
    ];
...
const siteconfig = {
    create: async (data) => {
        ...
        const sql = 'INSERT INTO siteconfig (siteName,clientUrl, logo, whiteLogo, icon, theme, instagramURL, facebookURL, twitterURL, linkedInURL, youtubeURL, mobile, email, googleReviewsEnabled, googlePlaceId, googleReviewsTitle, googleReviewsMax, googleReviewsMinRating, mapEmbedUrl, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())';
    ...
    update: async (id, data) => {
        await ensureGoogleReviewsColumns();
        const sqlUpdate = 'UPDATE siteconfig SET siteName = ?, clientUrl = ?, logo = ?, whiteLogo = ?, icon = ?, theme = ?, instagramURL = ?, facebookURL = ?, twitterURL = ?, linkedInURL = ?, youtubeURL = ?, mobile = ?, email = ?, googleReviewsEnabled = ?, googlePlaceId = ?, googleReviewsTitle = ?, googleReviewsMax = ?, googleReviewsMinRating = ?, mapEmbedUrl = ?, updated_at = NOW() WHERE id = ?';

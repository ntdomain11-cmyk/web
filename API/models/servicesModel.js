...
async function ensureServicesTable() {
  ...
  // Best-effort migration for existing tables missing thumbnailUrl
  try {
    await db.execute('ALTER TABLE services ADD COLUMN thumbnailUrl VARCHAR(500) NULL');
  } catch (err) {
...
async function execWithAutoCreate(sql, params) {
  try {
    ...
    // Auto-migrate missing thumbnailUrl column if needed
    if (
      err &&
      (err.code === 'ER_BAD_FIELD_ERROR' || err.errno === 1054) &&
      typeof err.sqlMessage === 'string' &&
      err.sqlMessage.includes("'thumbnailUrl'")
    ) {
      try {
        await db.execute('ALTER TABLE services ADD COLUMN thumbnailUrl VARCHAR(500) NULL');
      } catch (alterErr) {
        // Ignore if column already exists after a race; rethrow other errors
        if (!(alterErr && (alterErr.code === 'ER_DUP_FIELDNAME' || alterErr.errno === 1060))) {
          throw alterErr;
        }
      }

      // Retry original query after migration
      return await db.execute(sql, params);
    }
    throw err;
  }
...
const Services = {
  ...
  create: async (data) => {
    const sql =
      'INSERT INTO services (slug, title, shortText, iconKey, bulletsJson, outcomesJson, contentHtml, thumbnailUrl, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())';
  ...
  update: async (id, data) => {
    const sqlUpdate =
      'UPDATE services SET slug = ?, title = ?, shortText = ?, iconKey = ?, bulletsJson = ?, outcomesJson = ?, contentHtml = ?, thumbnailUrl = ?, updated_at = NOW() WHERE id = ?';

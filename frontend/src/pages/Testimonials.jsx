...
export default function Testimonials() {
  ...
  useEffect(() => {
    ...
    async function loadVideos() {
      try {
        const res = await apiGet('/api/video-testimonials/getAllVideoTestimonials')
        if (!mounted) return
        setVideoRows(Array.isArray(res?.data) ? res.data : [])
      } catch {
        if (!mounted) return
        setVideoRows([])
      }
  ...
  const videos = useMemo(() => (Array.isArray(videoRows) ? videoRows : []).filter((v) => v?.videoUrl), [videoRows])
  ...
  return (
    <section className="nt-testi">
      ...
      <div className="nt-testi__container">
        {videos.length ? (
          <div className="nt-testiVideo">
            <div className="nt-testiVideo__head">
              <div>
                <div className="nt-testi__kicker">Video testimonials</div>
                <div className="nt-testiVideo__title">Real stories from real clients</div>
              </div>
            </div>

            <div className="nt-testiVideo__grid">
              {videos.map((v) => (
                <article key={v.id} className="nt-testiVideo__card">
                  <div className="nt-testiVideo__media">
                    <video
                      className="nt-testiVideo__video"
                      controls
                      preload="metadata"
                  ...
                  <div className="nt-testiVideo__meta">
                    <div className="nt-testiVideo__name">{v.name || 'Client'}</div>
                    {v.designation ? <div className="nt-testiVideo__sub">{v.designation}</div> : null}
                    {v.title ? <div className="nt-testiVideo__desc">{v.title}</div> : null}
                  </div>
                </article>
              ))}
            </div>

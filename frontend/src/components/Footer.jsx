...
import { ArrowUp, Facebook, Instagram, Linkedin, Send, Youtube } from 'lucide-react'
...
export default function Footer() {
  ...
  const socials = useMemo(() => {
    const list = [
      ...
      { icon: XIcon, label: 'X', href: siteConfig?.twitterURL || 'https://x.com' },
  ...
  const onSubmit = async (e) => {
    ...
    setMessage('')
    try {
      await apiPost('/api/newsletter/subscribe', { email: trimmed })
      setStatus('success')
    ...
    <footer className="nt-footer">
      ...
      <div className="nt-footer__main">
        ...
        <div className="nt-footer__container">
              ...
              {status !== 'idle' && message ? (
                <div
                  style={{
                    marginTop: 8,
                    fontSize: 12,
                    color: status === 'success' ? 'rgba(0,128,0,0.9)' : 'rgba(180,32,32,0.9)',
                  }}
                >
                  {message}
                </div>
          ...
          <div className="nt-footer__copy">© Copyright NT Consultancy. Developed By <a href="https://aidges.com" target='_blank'>AIDGES INFOTECH.</a></div>
        </div>
      </div>
    </footer>
  )

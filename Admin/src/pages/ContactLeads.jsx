...
export default function ContactLeads() {
  ...
  const load = async ({ nextPage, nextSearch, nextPageSize } = {}) => {
    ...
    try {
      const res = await api.get('/api/leads/contact/getAllByPage', {
      ...
      setError(err?.response?.data?.error || err?.response?.data?.msg || 'Failed to load contact leads')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="nt-adminCard" style={{ padding: 16 }}>
      <PageHeader
        ...
        breadcrumbs={[{ label: 'Leads' }, { label: 'Contact' }]}
      />
